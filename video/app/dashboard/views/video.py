...
class ExternaVideo(View):
    ...
    def post(self, request):
        ...
        result = check_and_get_video_type(
            NationalityType, nationality, '非法的来源'
        )
        if result.get('code') != 0:
            return redirect('{}?error={}'.format(reverse('externa_video'), result['msg']))
...
class VideoSubView(View):
    ...
    def post(self, request, video_id):
        ...
        path_formats = '{}'.format(redirect(reverse('video_sub', kwargs={'video_id': video_id})))

        if not all(url):
            return redirect('{}?error={}'.format(path_formats, '缺少必要字段'))
        ...
        try:
            ...
            return redirect(reverse(path_formats, '创建失败'))
...
class VideoStarView(View):
    ...
    def post(self, request):
        ...
        if result.get('code') != 0:
            return redirect(reverse(path_format, result['msg']))
        ...
        try:
            ...
            return redirect(reverse(path_format, '创建失败'))
